...
void ReshapeGL (int width, int height)									// Reshape The Window When It's Moved Or Resized
{
	...
	glOrtho(0, (float)width, 0, (float)height, -800.0, 800.0);
...
BOOL CreateWindowGL (GL_Window* window)									// This Code Creates Our OpenGL Window
{
	...
	window->lastTickCount = GetTickCount64 ();							// Get Tick Count
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    *stdin = *hf_in;
	...
	ULONGLONG			tickCount;										// Used For The Tick Counter
	char8_t				title[20];
	strncpy(title,pGame->GetGameTitle(),19);
	...
	window.init.width			= pGame->GetScreenWidth();						// Window Width
	window.init.height			= pGame->GetScreenHeight();						// Window Height
	window.init.bitsPerPixel	= pGame->GetBitsPerPixel();						// Bits Per Pixel
	...
	while (g_isProgramLooping)											// Loop Until WM_QUIT Is Received
	{
		...
		if (CreateWindowGL (&window) == TRUE)							// Was Window Creation Successful?
		{
			...
			if (pGame->Initialize(&window, &keys) == FALSE)					// Call User Intialization
			{
				// Failure
				...
				while (isMessagePumpActive == TRUE)						// While The Message Pump Is Active
				{
					...
					else												// If There Are No Messages
					{
						...
						else											// If Window Is Visible
						{
							...
							pGame->UpdateFrame((uint32_t)(tickCount - window.lastTickCount));	// Update The Counter
							window.lastTickCount = tickCount;			// Set Last Count To Current Count
							pGame->DrawScene();			// Draw Our Scene
